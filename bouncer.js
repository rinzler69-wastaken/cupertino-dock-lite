...
export class Bouncer {
  ...
  _calculateBounceOffset(icon, iconSize, scaleFactor, dock_position) {
    let jX = 0, jY = 0;
    let scale = 1.0;
    let opacity = 255;
    ...
    // Calculate maximum bounce scale based on magnification setting.
    // At default magnify (0.5), maxBounceScale is 1.0.
    const maxBounceScale = magnify / 0.585;

    // Interpolate bounce scale between 1.0 (original default height when not magnified)
    // and maxBounceScale (proportional height when fully magnified) based on currentScale.
    const currentScale = icon._currentScale ?? 1.0;
    const peakScale = Math.max(1.1, 1.3 + magnify);
    let factor = 0;
    if (peakScale > 1.0) {
      factor = (currentScale - 1.0) / (peakScale - 1.0);
      factor = Math.max(0.0, Math.min(1.0, factor));
    }
    const bounceScale = 1.0 + (maxBounceScale - 1.0) * factor;

    if (icon._clickJump > 0) {
      let off = Math.sin(icon._clickJump * Math.PI) * iconSize * ANIM_ICON_RAISE * scaleFactor * 1.65 * ANIM_JUMP_HEIGHT * bounceScale;
      ...
      if (p <= scale_duration) {
        ...
        off = iconSize * ANIM_INTRO_HEIGHT * scaleFactor * bounceScale;
      } else {
        scale = 1.0;
        let drop_p = (p - scale_duration) / (1.0 - scale_duration);
        off = Math.cos(drop_p * Math.PI / 2) * iconSize * ANIM_INTRO_HEIGHT * scaleFactor * bounceScale;
      }
      if (dock_position === 'bottom') jY = -off; else if (dock_position === 'top') jY = off; else if (dock_position === 'left') jX = off; else if (dock_position === 'right') jX = -off;
    } else if (urgentBounceEnabled && icon._attentionJump > 0) {
      let off = Math.sin(icon._attentionJump * Math.PI) * iconSize * ANIM_ICON_RAISE * scaleFactor * 1.65 * ANIM_JUMP_HEIGHT * bounceScale;
      if (dock_position === 'bottom') jY = -off; else if (dock_position === 'top') jY = off; else if (dock_position === 'left') jX = off; else if (dock_position === 'right') jX = -off;
    }

    return { jX, jY, scale, opacity };
  }
  ...
  _animate() {
    ...
    icons.forEach((c) => {
      ...
      if (!found) {
        ...
        uiIcon.set_pivot_point(pivot.x, pivot.y); uiIcon._bin = bin; uiIcon._appwell = c._appwell; uiIcon._label = c._label;
    ...
    animateIcons.forEach((icon) => {
      ...
      icon.set_pivot_point(pivot.x, pivot.y);
    ...
    animateIcons.forEach((icon) => {
      ...
      const { jX, jY, scale, opacity } = this._calculateBounceOffset(icon, iconSize, scaleFactor, dock_position);
  ...
  _restoreCloneHandoffs() {
    ...
    this._iconsContainer.get_children().forEach(icon => {
      if (icon._bin && isActorAlive(icon._bin)) {
        if (icon._bin.first_child) icon._bin.first_child.opacity = 255;
      }
      this._setD2dBadgeOpacity(icon._appwell, 255);
  ...
  _restoreIcons() {
    this._findIcons().forEach(c => {
      if (!c || !c._bin || !isActorAlive(c._bin)) return;
      if (c._icon && isActorAlive(c._icon)) c._icon.opacity = 255;
      if (c._bin.first_child && isActorAlive(c._bin.first_child)) c._bin.first_child.opacity = 255;
      this._setD2dBadgeOpacity(c._appwell, 255);
      if (this.dashContainer && this.dash) {
        let sz = this.dash.iconSize * (this.extension.scale || 1.0);
        c._bin.set_width(sz);
        c._bin.set_height(sz);
        if (c._appwell?.get_parent() && isActorAlive(c._appwell.get_parent())) {
          c._appwell.get_parent().set_width(-1);
          c._appwell.get_parent().set_height(-1);
        }
      }
    });
    if (this.dash?._box && isActorAlive(this.dash._box)) {
      this.dash._box.get_children().forEach(c => {
        if (c.first_child && isActorAlive(c.first_child)) c.first_child.opacity = 255;
      });
    }
  }
